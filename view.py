...
with graph.as_default():    
    ...
    def individual_score_target_1():
        ...
        target_no = 1
        tres = mysession.query(Grouping.result).filter(Grouping.firer_id==firer_id).order_by(Grouping.datetimestamp.desc()).limit(5).all()[::-1]
        res=[]
        ten=[]
        tten=mysession.query(MPI.tendency_code).filter(MPI.firer_id==firer_id).order_by(MPI.datetimestamp.desc()).limit(5).all()[::-1]
        print(tres, file=sys.stderr)
        for ele in tres:
            for ele2 in ele:
                print(type(ele2), file=sys.stderr)
                res.append(ele2)
                
        for ele3 in tten:
            for ele4 in ele3:
                print(type(ele4), file=sys.stderr)
                ten.append(ele4)
    ...
    def individual_score_target_2():
        firer_id =mysession.query(TShooting.target_2_id).scalar()
        detail_no =mysession.query(TShooting.detail_no).scalar()
        session_no =mysession.query(TShooting.session_id).scalar()
        target_no = 2
        service_id = mysession.query(Shooter.service_id).filter(Shooter.id==firer_id).scalar()
    ...
    def prediction_target_1():
        ...
        if request.method == 'POST':
            firer_id,sess,o,p,u,q,t1_x,t1_y,xmpi,ympi,f,gp,Tfirt_x,Tfirt_y,fin_x_1,fin_y_1,result_1,fir_tendency_1=prediction_calculation_1()
            paper_ref=mysession.query(TPaper_ref.paper_ref).scalar()
            
            
            set_2_x=mysession.query(T_Firer_Details.final_x).filter(T_Firer_Details.date==curdate , T_Firer_Details.target_no==2 , T_Firer_Details.set_no==1 , T_Firer_Details.paper_ref==paper_ref , T_Firer_Details.session_id==sess).all()
            set_2_y=mysession.query(T_Firer_Details.final_y).filter(T_Firer_Details.date==curdate , T_Firer_Details.target_no==2 , T_Firer_Details.set_no==1 , T_Firer_Details.paper_ref==paper_ref , T_Firer_Details.session_id==sess).all()
            set_3_x=mysession.query(T_Firer_Details.final_x).filter(T_Firer_Details.date==curdate , T_Firer_Details.target_no==3 , T_Firer_Details.set_no==1 , T_Firer_Details.paper_ref==paper_ref , T_Firer_Details.session_id==sess).all()
            set_3_y=mysession.query(T_Firer_Details.final_y).filter(T_Firer_Details.date==curdate , T_Firer_Details.target_no==3 , T_Firer_Details.set_no==1 , T_Firer_Details.paper_ref==paper_ref , T_Firer_Details.session_id==sess).all()
            
            set_4_x=mysession.query(T_Firer_Details.final_x).filter(T_Firer_Details.date==curdate , T_Firer_Details.target_no==4 , T_Firer_Details.set_no==1 , T_Firer_Details.paper_ref==paper_ref , T_Firer_Details.session_id==sess).all()
            set_4_y=mysession.query(T_Firer_Details.final_y).filter(T_Firer_Details.date==curdate , T_Firer_Details.target_no==4 , T_Firer_Details.set_no==1 , T_Firer_Details.paper_ref==paper_ref , T_Firer_Details.session_id==sess).all()
            ...
            set_1_name=mysession.query(Shooter.name).filter(
                                                        Shooter.id==set_1_id
                                                        ).scalar()
            ...
            set_2_name=mysession.query(Shooter.name).filter(
                                                        Shooter.id==set_2_id
                                                        ).scalar()
            ...
            set_3_name=mysession.query(Shooter.name).filter(
                                                        Shooter.id==set_3_id
                                                        ).scalar()
            ...
            set_4_name=mysession.query(Shooter.name).filter(
                                                        Shooter.id==set_4_id
                                                        ).scalar()
    ...
    def prediction_calculation_1():
        ...
        target_no=1
        paper_ref=mysession.query(TPaper_ref.paper_ref).scalar()
        print(paper_ref ,file=sys.stderr)
        data_x_1=mysession.query(T_Firer_Details.final_x).filter(T_Firer_Details.date==curdate , T_Firer_Details.target_no==1 , T_Firer_Details.set_no==1 , T_Firer_Details.paper_ref==paper_ref , T_Firer_Details.session_id==sess_id).all()
        data_y_1=mysession.query(T_Firer_Details.final_y).filter(T_Firer_Details.date==curdate , T_Firer_Details.target_no==1 , T_Firer_Details.set_no==1 , T_Firer_Details.paper_ref==paper_ref , T_Firer_Details.session_id==sess_id).all()
        print(data_x_1,file=sys.stderr)
        set_no=mysession.query(TShooting.set_no).scalar()
        paper_ref=mysession.query(TShooting.paper_ref).scalar()
        print('Old x' ,file=sys.stderr)
        print(data_x_1,file=sys.stderr)
    ...
    def getfiringtendencytext(f1 ,firt_x,firt_y):
        ...
        if(isbullseye):
            ftcode="BE"
            fttext = "Bull's Eye "+"("+str(firt_y)+" , "+str(firt_x)+")"
        else:
    ...
    def update():
         ...
         if request.method == "POST":
             ...
             session ['tfirer_x1']=txf_list
             session ['tfirer_y1']=tyf_list
    ...
    def mpi(N,pointsarray):
        n=0
        pointsarrayfiltered=[]
        print('This is pointsarray',file=sys.stderr)
        print(pointsarray, file=sys.stderr)
        while (n<=100): 
            kmeans = KMeans(n_clusters=N, random_state=0).fit(pointsarray)
            center=kmeans.cluster_centers_ 
